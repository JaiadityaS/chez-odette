...
Register ONE trivial tool: `ping` → returns `"pong from Amélie's"`. Feature-detect. Add the dev test harness.
...
Build the real bakery page from DESIGN.md: hero with Amélie's voice, the story, today's bake, product grid. Static data from `lib/bakery.ts`. No new tools.
**Done when:** the page reads like a specific, warm, real bakery — brand/voice approved.

## Phase 3 — Soul tools
`get_todays_bake`, `the_story`, `recommend_for_occasion` — reading the data module, returning in Amélie's voice. The recommendation is **computed** (occasion/guests/prefs → choice + reasoning), NOT just text printed on the page — this is part of the honest "true failure surface" (PRD MUST #6).
**Done when:** an agent asked "what's good for an anniversary?" returns Amélie's voiced, computed recommendation, sourced from real app data.
...
**Done when:** an agent completes a mock order end-to-end AND the confirmation appears on Amélie's site; the sold-out item is correctly refused by `check_availability`.

## Phase 5 — Relationship + the two-door contrast
`join_regulars` (customer opts into Amélie's own list at order time). Build the cold aggregator-contrast page (supporting beat).
